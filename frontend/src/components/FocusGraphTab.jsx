...
// Node type visual config — deliberately kept distinct from task urgency palette
// Meeting: indigo/violet  Thread: amber  Person: cyan  Project: slate
const NODE_STYLE = {
  project: { color: "#1e293b", bg: "#f8fafc",  border: "#94a3b8", ring: "#475569" },
  meeting: { color: "#3730a3", bg: "#eef2ff",  border: "#a5b4fc", ring: "#6366f1" }, // indigo
  thread:  { color: "#92400e", bg: "#fef3c7",  border: "#fcd34d", ring: "#d97706" }, // amber
  task:    { color: "#1e3a5f", bg: "#f0f7ff",  border: "#93c5fd", ring: "#2563eb" }, // blue (fallback only)
  person:  { color: "#164e63", bg: "#ecfeff",  border: "#67e8f9", ring: "#0891b2" }, // cyan
};

// Task urgency colors — red/orange/yellow/green/gray only, no indigo/cyan/violet
const LANE_STYLE = {
  ...
  watch:     { dot: "#ca8a04", bg: "#fefce8", border: "#fde68a", text: "#713f12", label: "This Week" },
  on_track:  { dot: "#16a34a", bg: "#f0fdf4", border: "#86efac", text: "#14532d", label: "On Track" },
...
function buildGraph(projectOption, detail, focusGraphData) {
  ...
  // ── Step 1: Jira tasks first — canonical source of truth ──
  const jiraTaskSlugs = new Set();
  const fgTasks = (focusGraphData?.tasks || []).filter(t => t.projectName === projectOption.name);

  for (const t of fgTasks) {
    if (!t.context?.jira?.key) continue;
    const tid = `task:jira:${slugify(t.context.jira.key)}`;
    addNode(tid, "task", t.title, {
      owner: t.ownerName,
      dueDate: t.dueDate,
      lane: t.lane || "on_track",
      source: `Jira · ${t.context.jira.key}`,
      status: t.statusText,
    ...
    jiraTaskSlugs.add(slugify(t.title).slice(0, 40));
    const taskThread = t.context?.thread;
    if (taskThread?.conversationId) {
      const ltid = `thread:${slugify(taskThread.conversationId)}`;
      if (nodes.has(ltid)) addEdge(tid, ltid, "came_from");
    }
  }

  // Helper: true if candidate title significantly overlaps an existing Jira task
  const overlapsJira = (title) => {
    const s = slugify(title).slice(0, 40);
    if (jiraTaskSlugs.has(s)) return true;
    const words = s.split("_").filter(w => w.length > 3);
    if (!words.length) return false;
    for (const js of jiraTaskSlugs) {
      const jw = new Set(js.split("_").filter(w => w.length > 3));
      const shared = words.filter(w => jw.has(w)).length;
      if (shared / words.length >= 0.6) return true;
    }
    return false;
  };
  ...
  for (const m of (detail?.meetings || [])) {
    ...
    // Action items — skip any that are already covered by a Jira task
    for (const ai of (m.actionItems || [])) {
      if (!ai.task || overlapsJira(ai.task)) continue;
  ...
  // ── Step 4: Non-Jira fg tasks (email, reminders, todos) — non-overlapping ──
  for (const t of fgTasks) {
    if (t.context?.jira?.key) continue;
    if (overlapsJira(t.title)) continue;
    ...
    if (taskMeeting?.subject) {
      const lmid = `meeting:${slugify(taskMeeting.id || taskMeeting.subject)}`;
      if (nodes.has(lmid)) addEdge(tid, lmid, "created_in");
    }
  }

  // ── Step 5: Pending approval queue — non-overlapping ──
  for (const t of (detail?.pendingTasks || [])) {
    const label = t.label || t.data?.title || "Pending task";
    if (overlapsJira(label)) continue;
    const tid = `task:pending:${slugify(t.id || label || "").slice(0, 30)}`;
    if (nodes.has(tid)) continue;
    addNode(tid, "task", label, {
      ...
      status: "pending",
      description: t.data?.commitment || t.data?.body || label || "",
    });
    addEdge(tid, projId, "belongs_to");
  }
...
function GraphNode({ node, pos, isActive, isDimmed, onMouseDown }) {
  ...
  const meta = (() => {
    ...
    if (node.type === "person")  return node.metadata.email ? `@${node.metadata.email.split("@")[0]}` : "";
  ...
  return (
    ...
    >
      ...
      {meta && <div className="fg-node__meta">{meta}</div>}
    </div>
  );
}
...
export default function FocusGraphTab({ onGoDeeper }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getProjectDetails(opt.threadId || "", opt.name, opt.nextMeetingId || "", 20);
